 ...
 * of the License.
*/
#ifndef __LIST_HD_INCL__
#define __LIST_HD_INCL__
...
#define _list_insert_(headptr, ptr) \
    do { (ptr)->next = (headptr); \
        if (headptr) { (ptr)->prev = (headptr)->prev; \
        if ((headptr)->prev) (headptr)->prev->next = (ptr); (headptr)->prev = (ptr);} else (ptr)->prev = NULL; \
    } while (0)

#define _list_append_(tailptr, ptr) \
    do { (ptr)->prev = (tailptr); \
        if (tailptr) { (ptr)->next = (tailptr)->next; \
        if ((tailptr)->next) (tailptr)->next->prev = (ptr); (tailptr)->next = (ptr); } else (ptr)->next = NULL; \
    } while (0)

#define _list_remove_(ptr) \
    do { \
        if ((ptr)->prev) (ptr)->prev->next = (ptr)->next; \
        if ((ptr)->next) (ptr)->next->prev = (ptr)->prev; (ptr)->prev = (ptr)->next = NULL; \
    } while (0)
...
#define list_tail_obj(head_obj_ptr, obj_type, lh_var, tail_obj_ptr) \
    do {obj_type *obj_ptr; for (obj_ptr = head_obj_ptr; obj_ptr != NULL; \
        tail_obj_ptr = obj_ptr, obj_ptr = list_next_obj(obj_ptr, obj_type, lh_var));} while (0)

#define list_for_each(obj_ptr, head_obj_ptr, obj_type, lh_var) \
    for (obj_ptr = head_obj_ptr; obj_ptr != NULL; obj_ptr = list_next_obj(obj_ptr, obj_type, lh_var))
...
#define list_free(head_obj_ptr, obj_type, lh_var) \
    { obj_type *cur, *next; cur = head_obj_ptr;\
        while (cur) { next = list_entry(obj_type, cur->lh_var.next, lh_var); free(cur);cur = next;}}
